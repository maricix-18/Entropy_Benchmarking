...
#include "../../QuEST/QuEST/src/QuEST_internal.h"
#include <stdio.h>
#include <iostream>
#include <stdint.h>
#include <string.h>
...
int main()
{
  ...
  for (int qubits = 13; qubits <= qubits_max; qubits ++)
  {
    // time round
    ...
    for (int i = 0; i <= depth_max; i++)
    {
      ...
      for (int j = 0; j < qubits; j++)
      {
        ...
        densmatr_mixDepolarising(density_matrix_qreg, j, p1);
        angl_pos++;
      }
      ...
      for (int g = 0; g < 2; g++)
      {
        for (int j = g; j < qubits-1; j+=2)
        {
          ...
          densmatr_mixTwoQubitDepolarising(density_matrix_qreg, j, j+1,p2);
        }
      }

      // compute metrics
      cout<< "Get eingenmat for layer " << i << endl;
      ...
      */
      ...
      // cout<< "Compute eingenvalues for layer " << i << endl;
      // ces.compute(eing_mat, false); // compute only eingenvalues, not eigenvectors 
      ...
      cout<<"Append to json for layer " << i << endl;
      double vNd = 0;//abs(sum_entropy/double(qubits));  // don't calculate the vonneumann entropy
