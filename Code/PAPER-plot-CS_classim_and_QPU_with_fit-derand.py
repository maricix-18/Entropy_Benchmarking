...
Experimental results with derandomized classical shadows
"""
import argparse
import numpy as np
...
#CS_params = CSParams(num_samples=3, num_groups=1, M=3**num_qubits, K=1000, protocol_choice='derandomized')
...
# ========================= Experiment3 (Rigetti_QPU - derand) =============================
...
# Get metrics
metrics_exact = load_from_json(experiment4.metrics_file)
...
depth_tab = compute_depth_tab(circuit_params3.depth_min, circuit_params3.depth_max, circuit_params3.depth_step)
depth_tab_more_points = compute_depth_tab_more_points(circuit_params3.depth_min, circuit_params3.depth_max)
xticks = compute_xticks(depth_tab, circuit_params3.depth_step)
...
filename = 'n%d_M%d_K%d_grps%d_spls%d-derand.pdf' % (num_qubits, CS_params.M, CS_params.K, CS_params.num_groups, CS_params.num_samples)
...
# heuristic model
puri_model_QPU = [purity_model_globalDP_CS_circuit_measerr(num_qubits, depth, alpha_1_optim_QPU, alpha_2_optim_QPU, beta_optim_QPU) for depth in depth_tab_more_points]
puri_model_classim = [purity_model_globalDP_CS_circuit_measerr(num_qubits, depth, alpha_1_optim_classim, alpha_2_optim_classim, beta_optim_classim) for depth in depth_tab_more_points]
puri_model_classim_measerr = [purity_model_globalDP_CS_circuit_measerr(num_qubits, depth, alpha_1_optim_classim_measerr, alpha_2_optim_classim_measerr, beta_optim_classim_measerr) for depth in depth_tab_more_points]
plt.plot(depth_tab_more_points, puri_model_QPU, color="goldenrod", label="QPU model 1 - $\\alpha_1, \\alpha_2, \\beta$ = {:.4f}, {:.4f}, {:.4f}".format(alpha_1_optim_QPU, alpha_2_optim_QPU, beta_optim_QPU))
plt.plot(depth_tab_more_points, puri_model_classim, color="steelblue", label="sim model - $\\alpha_1, \\alpha_2, \\beta$ = {:.4f}, {:.4f}, {:.4f}".format(alpha_1_optim_classim, alpha_2_optim_classim, beta_optim_classim))
plt.plot(depth_tab_more_points, puri_model_classim_measerr, color="darkblue", label="sim model meas err - $\\alpha_1, \\alpha_2, \\beta$ = {:.4f}, {:.4f}, {:.4f}".format(alpha_1_optim_classim_measerr, alpha_2_optim_classim_measerr, beta_optim_classim_measerr))
# classical shadows
plt.errorbar(x=depth_tab, y=short_metrics['all_pur_mean_diff_n'], yerr=short_metrics['all_pur_std_diff_n'], ecolor="goldenrod", capsize=3, ls='none', label="QPU shadows")
...
plt.figure(figsize=(15, 6))
# heuristic model
R2d_model_QPU = [renyi_entropy_from_purity(puri)/num_qubits for puri in puri_model_QPU]
R2d_model_classim = [renyi_entropy_from_purity(puri)/num_qubits for puri in puri_model_classim]
R2d_model_classim_measerr = [renyi_entropy_from_purity(puri)/num_qubits for puri in puri_model_classim_measerr]
...
# classical shadows
plt.errorbar(x=depth_tab, y=short_metrics['all_R2d_mean_diff_n'], yerr=short_metrics['all_R2d_std_diff_n'], ecolor="goldenrod", capsize=3, ls='none', label="QPU shadows")
