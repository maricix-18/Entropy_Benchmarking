...
def plot_metric_DensMat(metric, metrics, plot_dir, show_maximally_mixed_state, save):
    if metric == 'Renyi-2 entropy density':
        metric_short = 'R2d'
    ...
    qubits=10
    num_qubits_min=qubits
    num_qubits_max=qubits
    num_qubits_step=1
    ...
    plt.figure()
    d = np.arange(depth_min, depth_max+1, depth_step)
    for num_qubits in range(num_qubits_min, num_qubits_max+1, num_qubits_step):
        if num_qubits == 1 and not metric == 'von Neumann entropy density':
            if metric == 'Purity':
                metric_lim = (1/4)*(1+p_AD1/(1 - (1 - p_DP1)*(1 - p_AD1)))**2 + (1/4)*(1/(1 + p_AD1/(p_DP1 * (1 - p_AD1))))**2
            if metric == 'Renyi-2 entropy density':
                pur_lim = (1/4)*(1+p_AD1/(1 - (1 - p_DP1)*(1 - p_AD1)))**2 + (1/4)*(1/(1 + p_AD1/(p_DP1 * (1 - p_AD1))))**2    
                metric_lim = -np.log2(pur_lim)/num_qubits
            plt.plot(d, [metric_lim]*len(d), linestyle='--', color='black', label='predicted limit')
        plt.plot(d, metrics['all_'+metric_short+'_diff_n'][int((num_qubits-num_qubits_min)/num_qubits_step)], label='$n = $' + str(num_qubits))
    ...
    plt.legend()
    plt.xlabel('Depth')
    xticks = compute_xticks(d, depth_step)
    plt.xticks(xticks)
    plt.ylabel(metric)
    #plt.ylim(0,1.1)
    if save:
        filename = metric_short + '-' + str(qubits) + 'D' + str(depth_max) + '_' + "Aer" + '.pdf'
        plt.savefig(plot_dir + filename)
    plt.show()
    return

def plot_metrics_DensMat ( metrics, show_maximally_mixed_state, plot_dir, save=True):
    """
    ...
    plot_metric_DensMat('von Neumann entropy density',  metrics, plot_dir, show_maximally_mixed_state, save)
    plot_metric_DensMat('Purity',  metrics, plot_dir, show_maximally_mixed_state, save)
    plot_metric_DensMat('Renyi-2 entropy density', metrics, plot_dir, show_maximally_mixed_state, save)
